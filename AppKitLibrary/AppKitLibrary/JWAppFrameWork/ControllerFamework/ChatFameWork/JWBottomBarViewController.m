...
- (void)viewDidLoad {
    ...
    NSDictionary *views = @{ @"bottomBar": self.editingBar};
    [self.view addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|[bottomBar]|" options:0 metrics:nil views:views]];
...
- (void)setup
{
    ...
    _emojiPageVC = [[JWEmojiPageVC alloc] initWithTextView:_editingBar.editView];
...
- (void)addBottomBar
{
    ...
    if (_hasAModeSwitchButton) {
        ...
        __weak JWBottomBarViewController *weakSelf = self;
...
{
    if ([_editingBar.editView isFirstResponder]) {
        [_editingBar.editView resignFirstResponder];
    }
}
