...
@interface JWEmojiPanelViewController ()<UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>

@property (nonatomic, strong) UICollectionView *emojiCollectionView;

@end

@implementation JWEmojiPanelViewController
...
- (void)viewDidLoad
{
    ...
    _emojiCollectionView.backgroundColor = [UIColor JWthemeColor];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (section == [self numberOfSectionsInCollectionView:collectionView] - 1&&
        ...
        [cell setBackgroundView:[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"jw_delete"]]];
    } else {
        NSInteger emojiNum = _pageIndex * 20 + section * 7 + row + 1;
        NSString *emojiImageName;
        if (emojiNum >= 106) {
            emojiImageName = [JWChatUtil.emojiDict[@(emojiNum).stringValue] stringByTrimmingCharactersInSet:[NSCharacterSet characterSetWithCharactersInString:@":"]];
        } else {
            emojiImageName = [NSString stringWithFormat:@"Expression_%ld", (long)emojiNum];
        }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (section == 2 && row == 6) {
        ...
        if (emojiNum >= 106) {
            emojiStr =  JWChatUtil.emojiDict[@(emojiNum).stringValue];
