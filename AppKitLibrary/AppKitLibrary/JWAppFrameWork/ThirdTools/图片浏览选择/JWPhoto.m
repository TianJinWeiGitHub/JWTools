...
+ (JWPhoto *)photoWithImage:(UIImage *)image {
    return [[JWPhoto alloc] initWithImage:image];
}

+ (JWPhoto *)photoWithURL:(NSURL *)url {
    return [[JWPhoto alloc] initWithURL:url];
}

+ (JWPhoto *)photoWithAsset:(PHAsset *)asset targetSize:(CGSize)targetSize {
    return [[JWPhoto alloc] initWithAsset:asset targetSize:targetSize];
}

+ (JWPhoto *)videoWithURL:(NSURL *)url {
    return [[JWPhoto alloc] initWithVideoURL:url];
}
                                                  ...
                                                  progress:^(NSInteger receivedSize, NSInteger expectedSize) {
                                                      if (expectedSize > 0) {
                                                          ...
                                                          NSDictionary* dict = [NSDictionary dictionaryWithObjectsAndKeys:
                                                                                ...
                                                                                self, @"photo", nil];
                                                          [[NSNotificationCenter defaultCenter] postNotificationName:JWPHOTO_PROGRESS_NOTIFICATION object:dict];
                                                      }
                                                  }
                                                 completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL) {
                                                     if (error) {
                                                         JWLog(@"SDWebImage failed to download image: %@", error);
                                                     }
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        @autoreleasepool {
            @try {
                ...
                if (!_underlyingImage) {
                    JWLog(@"Error loading photo from path: %@", url.path);
                }
            } @finally {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        @autoreleasepool {
            @try {
                if (SYSTEM_VERSION_LESS_THAN(@"8")) {
                    ...
                    [assetslibrary assetForURL:url
                                  ...
                                  failureBlock:^(NSError *error) {
                                      self.underlyingImage = nil;
                                      JWLog(@"Photo from asset library error: %@",error);
                                      [self performSelectorOnMainThread:@selector(imageLoadingComplete) withObject:nil waitUntilDone:NO];
                                  }];
                }else{
                ...
                JWLog(@"Photo from asset library error: %@", e);
                [self performSelectorOnMainThread:@selector(imageLoadingComplete) withObject:nil waitUntilDone:NO];
            }
        }
    });
...
- (void)postCompleteNotification {
    [[NSNotificationCenter defaultCenter] postNotificationName:JWPHOTO_LOADING_DID_END_NOTIFICATION
                                                        object:self];
}
